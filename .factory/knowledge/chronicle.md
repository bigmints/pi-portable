...
- **2026-05-30/31 | Model Appearance Settings:** Delivered `useModelSettingsStore` with `localStorage` persistence. Implemented `ModelSelector`, `AppearanceSettings`, `ThemeToggle`, `ModelSettingsPanel`. Integrated into root layout & settings page. Passed all build gates.
...
- **2026-05-31 | Design Tokens & PWA Hardening:** Overhauled core design tokens. Gated PWA/local-storage initialization logic to prevent SSR crashes. Resolved `useContext` prerendering failures in `.next/server/chunks`.
