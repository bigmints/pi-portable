/**
 * Projects store — project list management with caching and CRUD
 */
...
export const useProjectStore = create<ProjectsState>((set, get) => ({
  ...
  fetchProjects: async () => {
    ...
    try {
      ...
      const activeProject = projects.find((p: Project) => p.isActive) || null;
  ...
  switchProject: async (id: string) => {
    ...
    try {
      ...
      const data = await res.json();
      set(() => ({
        projects: data.projects || updatedProjects,
        activeProject: data.activeProject || newActive,
        lastFetch: Date.now(),
      }));
    } catch (err) {
      // Revert on failure
